...
function Login({onLogin}){
  ...
  async function google(){setError(""); if(!supabaseConfigured){setError("Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to enable Google sign-in."); return;} localStorage.setItem("smartlogix_pending_role",role); const {error:authError}=await supabase.auth.signInWithOAuth({provider:"google",options:{redirectTo:window.location.origin}}); if(authError) setError(authError.message)}
